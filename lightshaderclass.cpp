...
bool LightShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	LPCWSTR filenameLlightPixel = L"light.ps";
	LPCWSTR filenameLlightVertex = L"light.vs";
	result = InitializeShader(device, hwnd, filenameLlightVertex, filenameLlightPixel);
	if(!result)
	{
		return false;
	}
...
bool LightShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "failed to compile \n not find the shader Vertex Light file", "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "failed to compile \n not find the shader Pixel Light file", "Missing Shader File", MB_OK);
		}
