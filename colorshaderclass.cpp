...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	LPCWSTR filenameColorShaderPixel = L"color.ps";
	LPCWSTR filenameColorShaderVertex = L"color.vs";

	result = InitializeShader(device, hwnd, filenameColorShaderVertex, filenameColorShaderPixel);
	if(!result)
	{
		return false;
	}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "not find the file color vertex", "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "not find the file color shader", "Missing Shader File", MB_OK);
		}
...
void ColorShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	...
	MessageBox(hwnd, "Error compiling shader color. ", " Check shader-error.txt for message.", MB_OK);
