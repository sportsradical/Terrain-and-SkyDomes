...
bool ApplicationClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	if(!result)
	{
		MessageBox(hwnd, "Could not initialize the texture manager object.", "Error", MB_OK);
		return false;
	}

	// Load textures into the texture manager.
	LPCWSTR filenameTextureManager[] = {
		L"data/textures/dirt01d.tga",
		L"data/textures/dirt01n.tga"
	};


	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), filenameTextureManager[0], 0);
	if(!result)
	{ 
		return false; 
	}

	result = m_TextureManager->LoadTexture(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), filenameTextureManager[1], 1);
	if(!result)
	{
		return false;
	}
