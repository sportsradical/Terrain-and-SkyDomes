...
{
	bool result;
	LPCWSTR videoCard[128];
	int videoMemory;
	LPCWSTR videoString[144];
	LPCWSTR memoryString[32];
	LPCWSTR tempString[16];
	int i;
	...
	LPCWSTR filenameFonteInterFaceText = L"data/font/font01.txt";
	LPCWSTR filenameFonteInterFaceTexture = L"data/font/font01.tga";
	result = m_Font1->Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), filenameFonteInterFaceText,
						filenameFonteInterFaceTexture, 32.0f, 3);
	if (!result)
	{
		return false;
	}
	...
	//						256, false, m_Font1, videoString, 10, 10, 1.0f, 1.0f, 1.0f);
	//if(!result)
	//{ 
	//	return false; 
	//}
	...
	result = m_PositionStrings[3].Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), screenWidth, screenHeight, 16, false, m_Font1, 
											 L"rX: 0", 10, 370, 1.0f, 1.0f, 1.0f);
	if(!result)
	{ 
		return false;
	}

	result = m_PositionStrings[4].Initialize(Direct3D->GetDevice(), Direct3D->GetDeviceContext(), 
					screenWidth, screenHeight, 16, false, m_Font1, L"rY: 0", 10, 390, 1.0f, 1.0f, 1.0f);
	if(!result)
	{ 
		return false; 
	}
											   ...
											   float rotX, float rotY, float rotZ)
{
	...
	//	_itoa_s(std::to_string(rotationY), ((char)(tempString)), 10);
