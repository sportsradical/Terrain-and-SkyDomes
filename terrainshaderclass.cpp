...
bool TerrainShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	LPCWSTR terrainPixelShader = L"terrain.ps";
	LPCWSTR terrainVertexShader = L"terrain.vs";
	result = InitializeShader(device, hwnd, terrainVertexShader, terrainPixelShader);
	if(!result)
	{
		return false;
	}
...
bool TerrainShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "shader failed to compile \n not find the Terrain vertex shader file", "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "shader failed to compile \n not find the Terrain Pixel shader file", "Missing Shader File", MB_OK);
		}
...
void TerrainShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	...
	MessageBox(hwnd, "Error compiling Terrain shader. ", " Check shader-error.txt for message.", MB_OK);
