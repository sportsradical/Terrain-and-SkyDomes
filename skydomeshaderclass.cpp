...
bool SkyDomeShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	LPCWSTR filenameSkyDomesPixel = L"skydome.ps";
	LPCWSTR filenameSkyDomesVertex = L"skydome.vs";
	result = InitializeShader(device, hwnd, filenameSkyDomesVertex, filenameSkyDomesPixel);
	if(!result)
	{
		return false;
	}
...
bool SkyDomeShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "the shader failed to compile \n not find the Vertex Shader SkyDome file", "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "the shader failed to compile \n not find the Pixel Shader SkyDome file", "Missing Shader File", MB_OK);
		}
...
void SkyDomeShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	char* compileErrors;
