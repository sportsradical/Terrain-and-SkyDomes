...
bool FontShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	LPCWSTR filenameFontVertex = L"font.vs";
	LPCWSTR filenameFontShader = L"font.ps";
	result = InitializeShader(device, hwnd, filenameFontVertex, filenameFontShader);
	if(!result)
	{
		return false;
	}
...
bool FontShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, LPCWSTR vsFilename, LPCWSTR psFilename)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "failed to compile font Vertex Shader \n not find the shader file", "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "failed to compile font Pixel Shader \n not find the shader file", "Missing Shader File", MB_OK);
		}
...
void FontShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, LPCWSTR shaderFilename)
{
	char* compileErrors;
	...
	MessageBox(hwnd, "Error compiling shader Font. ", " Check shader-error.txt for message.", MB_OK);
